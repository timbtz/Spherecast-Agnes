---
name: e2e-test
description: Comprehensive end-to-end testing command. Launches parallel sub-agents to research the codebase (structure, database schema, potential bugs), then uses the Vercel Agent Browser CLI (or Playwright Python as fallback) to test every user journey — taking screenshots, validating UI/UX, and querying the database to verify records. Run after implementation to validate everything before code review.
disable-model-invocation: true
---

## Project-specific conventions (HappyRobot / TUM.ai Makeathon)

- **Screenshots must be saved to:** `testing/frontend/screenshots/` (relative to project root)
  - Full path: `/home/developer/Projects/TUM.ai Makeathon/testing/frontend/screenshots/`
  - Naming: two-digit step prefix + descriptive name, e.g. `01_initial_load.png`, `09_hans_weber_detail.png`
- **Backend URL:** `http://localhost:9000` (start with `uv run uvicorn api.main:app --port 9000`)
  - Ports 8000 and 8080 are already occupied by other services on this machine.
- **Browser tooling:** `agent-browser` is installed and works natively. For scripted/bulk testing, Playwright Python also works:
  ```bash
  python3 testing/frontend/test_dashboard.py
  ```
- **Existing test suite:** `testing/frontend/test_dashboard.py` — 35/37 tests passing. Extend this file rather than starting from scratch.
- **Known issue:** `POST /pipeline/run/*` returns 500 because `WEBHOOK_SECRET` env var is unset in dev. Set it before testing pipeline trigger flows.
- **Database:** SQLite at `wiki/happyrobot.db` — query with `sqlite3 wiki/happyrobot.db "SELECT ..."` for DB validation steps.
